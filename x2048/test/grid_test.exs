defmodule X2048.GridTest do
  use ExUnit.Case, async: true

  describe "squash_tiles/1 squash line to left: " do
    test "2 2 2 -> 4 2 nil" do
      assert X2048.Grid.squash_tiles([2, 2, 2]) == [4, 2, nil]
    end

    test "2 2 2 2 -> 4 4 nil nil " do
      assert X2048.Grid.squash_tiles([2, 2, 2, 2]) == [4, 4, nil, nil]
    end

    test "4 4 4 4 4 4 -> 8 8 8 nil nil nil" do
      assert X2048.Grid.squash_tiles([4, 4, 4, 4, 4, 4]) == [8, 8, 8, nil, nil, nil]
    end

    test "2 4 nil 4 -> 2 8 nil nil" do
      assert X2048.Grid.squash_tiles([2, 4, nil, 4]) == [2, 8, nil, nil]
    end

    test "2 -> 2" do
      assert X2048.Grid.squash_tiles([2]) == [2]
    end

    test "[] -> []" do
      assert X2048.Grid.squash_tiles([]) == []
    end
  end
  ...
  describe "split_by_obstacle/1 chops line to the sublines by obstacle" do
    test "2 4 o 2 x 2 o x -> 2 4 o 2 | x | 2 o | x |" do
      actual = X2048.Grid.split_by_obstacle([2, 4, nil, 2, :obstacle, 2, nil, :obstacle])
      expected = [[2, 4, nil, 2], [:obstacle], [2, nil], [:obstacle], []]
      assert actual == expected
    end

    test "o x x 2 o -> o | x | x | 2 o" do
      actual = X2048.Grid.split_by_obstacle([nil, :obstacle, :obstacle, 2, nil])
      expected = [[nil], [:obstacle], [], [:obstacle], [2, nil]]
      assert actual == expected
    end

    test "x 2 4 x o -> | x | 2 4 | x | o" do
      actual = X2048.Grid.split_by_obstacle([:obstacle, 2, 4, :obstacle, nil])
